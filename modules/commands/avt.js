...
module.exports.run = async ({ api, event, args, Users }) => {
    ...
    const sendImage = async (url, message) => {
        try {
            ...
            if (response.status !== 200) {
                return api.sendMessage("⚠️ Không thể tải ảnh đại diện. يرجى thử lại sau.", threadID, messageID);
            }
    ...
    if (args[0] === "box") {
        ...
        try {
            ...
            console.error(error);
            return api.sendMessage("⚠️ Không thể lấy thông tin của box. يرجى thử lại sau.", threadID, messageID);
        }
    }
    ...
    if (args[0] === "user") {
        ...
        try {
            ...
            console.error(error);
            return api.sendMessage(`⚠️ Không thể lấy thông tin người dùng. يرجى thử lại sau.`, threadID, messageID);
        }
    }
};
