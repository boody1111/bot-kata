...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!query) {
    api.sendMessage("⚠️ يرجى إدخال từ khóa tìm kiếm", threadID, messageID);
    return;
  }
  ...
  try {
    ...
    if (dataaa.length === 0) {
      api.sendMessage(`❎ لم يتم العثور على kết quả cho từ khóa "${query}"`, threadID, messageID);
      return;
    }
    ...
    api.sendMessage(`❎ حدث خطأ trong quá trình tìm kiếm`, threadID, messageID);
  }
};
...
module.exports.handleReply = async function ({ event, api, handleReply, args }) {
  ...
  switch (handleReply.type) {
    case 'choosee':
      ...
      if (isNaN(choose)) {
        return api.sendMessage('⚠️ يرجى إدخال 1 con số', tid, mid);
      }
