module.exports = function ({ api, models }) {
  setInterval(function () {
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ بيانات ]');
            ...
            logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen', 'successLoadEnvironment'), '[ بيانات ]');
        } catch (error) {
            return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
        }
    }());
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝑨𝑳𝑰𝑿 𝑩𝑶𝑻" : global.config.BOTNAME}`, "[ معلومات البوت ]");
  ...
  const handleRefresh = require("./handle/handleRefresh")({  api, Threads, Users, Currencies, models });
    ...
    const checkAndExecuteEvent = async () => {
        ...
        /*smol check*/
        if (!fs.existsSync(require('path').dirname(datlichPath))) fs.mkdirSync(require('path').dirname(datlichPath), { recursive: true });
