...
const getChatResponse = async (text, api, event) => {
    ...
    try {
        ...
        if (response.data && response.data.result) {
            ...
            return "لا يوجد kết quả từ API.";
        }
    } catch {
        return "حدث خطأ khi kết nối tới API.";
    }
};
...
module.exports.run = async ({ api, event, args }) => {
    try {
        ...
        if (!text) {
            return api.sendMessage('يرجى كتابة نص للدردشة مع الذكاء الاصطناعي.', event.threadID, event.messageID);
        }
...
module.exports.handleReply = async ({ api, event }) => {
    try {
        ...
        if (sessionThreadID && sessionThreadID === event.threadID) {
            ...
            if (!text || text.trim() === '') {
                return api.sendMessage('يرجى كتابة نص để tiếp tục trò chuyện.', event.threadID, event.messageID);
            }
