class get2fa {
    ...
    async onLoad(){
      try {
          ...
          const { existsSync, writeFileSync, readFileSync } = fsExtra;
          ...
          fsExtra.ensureDirSync(dirname(path));
    ...
    const { existsSync, writeFileSync, readFileSync } = fsEx;
    const pathData = join(__dirname, "cache", "data", "get2fabyJudas.json");
    fsEx.ensureDirSync(join(__dirname, "cache", "data"));
    if (!existsSync(pathData)) writeFileSync(pathData, JSON.stringify([]), "utf-8");
