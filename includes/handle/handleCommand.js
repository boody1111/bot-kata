module.exports = function ({ api, models, Users, Threads, Currencies }) {
 ...
 return api.sendMessage("⛔ هذا البوت متاح لمشرف البوت فقط في المحادثات الخاصة!", threadID, messageID)
}    
   if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
     if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) return api.sendMessage('⛔ هذا البوت مخصص لمشرف البوت فقط!', threadID, messageID)
   }
    if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) { 
         return api.sendMessage('⛔ هذا البوت مخصص للمطورين فقط!', threadID, messageID)

       }
if (global.moduleData && global.moduleData.adminOnlyThreads && global.moduleData.adminOnlyThreads[threadID]) {
    if (!ADMINBOT.includes(senderID) && !NDH.includes(senderID)) return;
}
let dataAdbox = { adminbox: {} };
try {
  const _dataAdboxPath = require('path').resolve(__dirname, './../../modules/commands/cache/data.json');
  if (require('fs').existsSync(_dataAdboxPath)) dataAdbox = JSON.parse(require('fs').readFileSync(_dataAdboxPath, 'utf-8'));
} catch(e) {}
   var threadInf = {};
   try { threadInf = (threadInfo.get(threadID) || await Threads.getInfo(threadID)) || {}; } catch (e) {}
   const findd = (threadInf.adminIDs || []).find(el => el.id == senderID);
  if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true && !NDH.includes(senderID) && !findd && event.isGroup == true) return api.sendMessage('⛔ هذا الأمر متاح لمشرفي المجموعة فقط!', event.threadID, (err) => {}, messageID)
   ...
   if (event.isGroup == !![])
     try {
      ...
      var permssion = 0;
   var threadInfoo = {};
   try { threadInfoo = (threadInfo.get(threadID) || await Threads.getInfo(threadID)); } catch {}
