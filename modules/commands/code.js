...
module.exports.run = async ({ api, event, args, Users }) => {
  ...
  if (!permission.includes(event.senderID)) return api.sendMessage("?", event.threadID, event.messageID);
    ...
    switch (args[0]) {
        case '-c':
        case 'create': {
            if (args[1].length == 0) return api.sendMessage("يرجى đặt tên cho file của bạn", event.threadID);
  ...
  if(content.endsWith(".js") || content.endsWith(".json")) {
    var data = await fs.readFile(
      ...
      async function (err, data) {
        if (err) return api.sendMessage(`لم يتم العثور على file "${content}".`, threadID, messageID);
    ...
    if(!args[1]) return api.sendMessage('يرجى إدخال tên file muốn áp dụng code mới!', threadID, messageID);
    const options = {
      method: 'GET',
      url: event.messageReply.body
    };
    request(options, function (error, response, body) {
      if(error) return api.sendMessage('يرجى chỉ reply link (không chứa gì khác ngoài link)', threadID, messageID);
      ...
      load('.language-js').each((index, el) => {
        ...
        fs.writeFile(`${__dirname}/${args[1]}.js`, code, "utf-8",
          function(err) {
            if (err) return api.sendMessage(`حدث خطأ khi áp dụng code mới cho "${args[1]}.js".`);
    ...
    if(!fs.existsSync(__dirname+"/"+file)) { 
      ...
      var checker = stringSimilarity.findBestMatch(file, mdl)
        if (checker.bestMatch.rating >= 1) var search = checker.bestMatch.target;
          if(search == undefined) return api.sendMessage('لم يتم العثور على file ' + args.join(" "), event.threadID, event.messageID); 
      return api.sendMessage('لم يتم العثور على file: ' + file + ' \nNhưng có file gần giống là: ' + search + '.js, \n\nThả cảm xúc vào tin nhắn này để give nó.', event.threadID, (error, info) => {
