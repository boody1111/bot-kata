...
module.exports.run = async function ({ event, api, args }) {
    ...
    switch (args[0]) {
        case "add":
            ...
            if (thisThread.nameUser.length > 0) return api.sendMessage("→ Cấu hình cũ tồn tại! يرجى xóa cấu hình trước.", threadID, messageID);
        ...
        case "remove":
            if (thisThread.nameUser.length === 0) return api.sendMessage("→ لا يوجد cấu hình để xóa!", threadID, messageID);
