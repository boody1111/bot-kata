...
const { readdirSync, readFileSync, writeFileSync, existsSync, unlinkSync, rm } = fsExtra;
...
// ── إنشاء المجلدات الضرورية تلقائياً عند بدء التشغيل ──
const requiredDirs = [
    join(__dirname, 'modules', 'commands', 'cache'),
    join(__dirname, 'modules', 'commands', 'cache', 'data'),
    join(__dirname, 'modules', 'commands', 'cache', 'img'),
    join(__dirname, 'modules', 'commands', 'cache', 'image'),
    join(__dirname, 'modules', 'commands', 'cache', 'audio'),
    join(__dirname, 'modules', 'commands', 'cache', 'video'),
    join(__dirname, 'modules', 'commands', 'cache', 'tmp'),
    join(__dirname, 'modules', 'commands', 'data'),
    join(__dirname, 'modules', 'commands', 'tuongtac', 'checktt'),
    join(__dirname, 'tmp'),
    join(__dirname, 'temp'),
];
for (const dir of requiredDirs) {
    try { fsExtra.ensureDirSync(dir); } catch {}
}
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
            ...
            function () {
                ...
                for (const command of listCommand) {
                    try {
                        ...
                        if (module.config.aliases && Array.isArray(module.config.aliases)) {
                            for (const alias of module.config.aliases) {
                                if (!global.client.commands.has(alias)) global.client.commands.set(alias, module);
                            }
                        }
        ...
        logger.loader(`وقت التشغيل: ${((Date.now() - global.client.timeStart) / 1000).toFixed()} ثانية`)   
    ...
    try {
        try {
          ...
          where: filter  
            ...
            logger.loader('تعذّر الاتصال بقاعدة بيانات ANTI SETTING', '[ CONNECT ]');
            console.log(error);
        }
...
process.on('unhandledRejection', (err, p) => {
    ...
    const isMqttNoise = msg.includes('packet') || msg.includes('MQTT') || msg.includes('protocol') || msg.includes('read ECONNRESET') || msg.includes('write EPIPE');
    if (isMqttNoise) return;
    logger(msg.slice(0, 200), 'REJECTION');
});
