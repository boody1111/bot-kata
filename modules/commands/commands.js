...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      ...
      if (!args[1] || !["on", "off"].includes(args[1])) return reply('يرجى chọn on hoặc off');
      const form = {
        av: botID,
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (err, data) => {
        if (err || JSON.parse(data).errors) reply("حدث خطأ, vui lòng thử lại sau");
      ...
      api.logout((e) => {
        if (e) return reply('حدث خطأ, vui lòng thử lại sau');
    ...
    api.changeBio(bio, false, (err) => {
      if (err) return reply("حدث خطأ, vui lòng thử lại sau");
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      if (e) return reply(`حدث خطأ, vui lòng thử lại sau`);
      else if (JSON.parse(i).errors) reply(`حدث خطأ: ${JSON.parse(i).errors[0].summary}, ${JSON.parse(i).errors[0].description}`);
    ...
    else return reply(`يرجى إدخال link hình ảnh hợp lệ hoặc phản hồi tin nhắn kèm một ảnh muốn đặt làm avatar cho bot`, (err, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    try {
      ...
      if (uploadImageToFb.error) return reply("حدث خطأ: " + uploadImageToFb.error.errorDescription);
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
        if (e) reply(`حدث خطأ, vui lòng thử lại sau`);
        else if (JSON.parse(i.slice(0, i.indexOf('\n') + 1)).errors) reply(`حدث خطأ: ${JSON.parse(i).errors[0].description}`);
        else reply(`Đã thay đổi avatar cho bot thành công`);
      });
    }
    catch(err) {
      reply(`حدث خطأ, vui lòng thử lại sau`);
    }
  }


  else if (type == 'blockUser') {
    if (!body) return reply("يرجى إدخال uid của những người bạn muốn chặn trên messenger, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply("يرجى إدخال uid của những người bạn muốn bỏ chặn trên messenger, có thể nhập nhiều id cách nhau bằng dấu cách hoặc xuống dòng", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply("يرجى إدخال nội dung muốn tạo bài viết", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply('يرجى إدخال id của post bạn muốn comment', (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply('يرجى إدخال nội dung bạn muốn comment cho bài viết', (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    for (const postID of postIDs) {
      ...
      try {
        ...
        reply("حدث خطأ, id bài viết không tồn tại hoặc bạn không phải chủ của bài viết này");
      }
    ...
    if (!body) return reply(`يرجى إدخال id bài viết bạn muốn reaction`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!'unlike/like/love/heart/haha/wow/sad/angry'.split('/').includes(feeling)) return reply('يرجى chọn một trong những cảm xúc sau unlike/like/love/heart/haha/wow/sad/angry', (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
